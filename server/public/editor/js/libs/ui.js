...
class UIConfButton extends UIElement {
...
//bsm vareint
class UIBoxType extends UIDiv {
	constructor(defaultName, editor, variantName) {
		super();

		this.dom.className = "BoxType";

		// Create the input field for the default name
		this.defaultNameInput = new UIInput(defaultName || "");
		this.defaultNameInput.setWidth("90px");
		this.defaultNameInput.onChange(() => {
			// Do something when the default name is changed (if needed)
		});

		// Create a button to add a new variant field
		this.addVariantButton = new UIButton("Add Variant").setId("addVariantBtn");

		this.addVariantButton.onClick(async () => {
			const defaultName = this.defaultNameInput.getValue();
		...
		this.spinnerElement = new UIElement(document.createElement("div"));
		this.spinnerElement.dom.className = "spinner";
		this.add(this.defaultNameInput);
		this.add(this.addVariantButton);
	}
	addNewVariantField() {}
	addsignals(data, signals) {
		...
		this.remove(this.addVariantButton);
		this.remove(this.defaultNameInput);
		this.remove(this.spinnerElement);
	}
	...
	showErrorAlert() {
		alert("Error occurred while saving the configuration!");
	}
}
// ===============================================================================
